  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setMatches(matchesData.slice(0, 3)); // Only show top 3 upcoming matches
      } catch (err) {
        console.error(err);
  ...
  return (
    <div>
      <header style={{ marginBottom: '40px' }}>
        <h1 style={{ fontSize: '2.5rem', marginBottom: '8px' }}>¡Bienvenido de vuelta{user?.fullName ? `, ${user.fullName.split(' ')[0]}` : ''}!</h1>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', marginBottom: '40px' }}>
        <div className="glass-panel" style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '12px' }}>
            <span style={{ fontSize: '3rem', fontWeight: 800, color: 'var(--accent-primary)', lineHeight: 1 }}>{user?.totalPoints || 0}</span>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '12px' }}>
            <span style={{ fontSize: '3rem', fontWeight: 800, color: 'var(--accent-secondary)', lineHeight: 1 }}>--%</span>
            <span style={{ color: 'var(--text-secondary)', marginBottom: '8px' }}>En desarrollo</span>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '12px' }}>
            <span style={{ fontSize: '3rem', fontWeight: 800, color: '#eab308', lineHeight: 1 }}>02:14:00</span>
            <span style={{ color: 'var(--text-secondary)', marginBottom: '8px' }}>horas</span>
          </div>
        </div>
      </section>
            ...
            matches.map((match) => (
              <div key={match.id} className="glass-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px' }}>
                ...
                <div style={{ display: 'flex', gap: '16px', marginLeft: '48px' }}>
                  <button className="btn-primary" style={{ padding: '8px 16px', fontSize: '0.875rem' }} onClick={() => router.push(`/dashboard/matches/${match.id}`)}>Predecir</button>
                </div>
              </div>
            ))
          )}
